"use client";
import { useState } from "react";
import { useTranslations, useLocale } from "next-intl";
import { motion } from "framer-motion";
import Link from "next/link";
import { Phone, RefreshCw, ArrowRight, Tag, ShoppingCart, Check } from "lucide-react";
import { cn } from "@/lib/utils";
import { useCartStore } from "@/lib/cart-store";
import { DEAL_SECTIONS, dealItemToProduct, type DealSection } from "@/data/deals";
import type { Product } from "@/data/products";

export default function DealsClient() {
  const t = useTranslations("deals");
  const locale = useLocale();
  const [activeSection, setActiveSection] = useState<string | null>(null);
  const [addedId, setAddedId] = useState<string | null>(null);
  const addItem = useCartStore((s) => s.addItem);

  function handleAdd(itemId: string, product: Parameters<typeof addItem>[0]) {
    addItem(product, 1);
    setAddedId(itemId);
    setTimeout(() => setAddedId(null), 1500);
  }

  const visibleSections = DEAL_SECTIONS.filter(
    (s) => activeSection === null || s.title === activeSection
  );

  return (
    <div className="min-h-screen bg-[#fafaf8]">

      {/* Hero — same structure as specials page */}
      <div className="bg-stone-900 pt-24 pb-10 px-6">
        <div className="max-w-screen-xl mx-auto">
          <div className="flex items-center gap-2 mb-2">
            <Tag size={14} className="text-red-400" />
            <p className="text-red-400 text-sm font-semibold tracking-widest uppercase">Weekly Deals</p>
          </div>
          <h1 className="font-serif text-3xl md:text-4xl font-bold text-white mb-2">{t("heading")}</h1>
          <p className="text-stone-400 max-w-2xl">{t("subheading")}</p>

          {/* Filter buttons inside hero — same pattern as specials tabs */}
          <div className="mt-8 flex gap-2 flex-wrap">
            <button
              onClick={() => setActiveSection(null)}
              className={cn(
                "flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold transition-all",
                activeSection === null ? "bg-red-600 text-white shadow-lg" : "bg-white/10 text-stone-300 hover:bg-white/20"
              )}
            >
              All
              <span className="bg-white/20 rounded-full px-2 py-0.5 text-xs">
                {DEAL_SECTIONS.reduce((a, s) => a + s.items.length, 0)}
              </span>
            </button>
            {DEAL_SECTIONS.map((s) => (
              <button
                key={s.title}
                onClick={() => setActiveSection(s.title === activeSection ? null : s.title)}
                className={cn(
                  "flex items-center gap-1.5 px-4 py-2 rounded-full text-sm font-semibold transition-all",
                  activeSection === s.title ? "bg-red-600 text-white shadow-lg" : "bg-white/10 text-stone-300 hover:bg-white/20"
                )}
              >
                <span>{s.icon}</span>
                {s.title}
              </button>
            ))}
          </div>

          <div className="mt-4 flex flex-wrap gap-2">
            <a
              href="tel:+12156275323"
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white/10 hover:bg-white/20 text-stone-300 rounded-full text-xs transition-colors"
            >
              <Phone size={11} />
              (215) 627-5323
            </a>
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-white/10 text-stone-300 rounded-full text-xs">
              <RefreshCw size={11} />
              Updated weekly
            </div>
            <Link
              href={`/${locale}/ordering/guest`}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white/10 hover:bg-white/20 text-stone-300 hover:text-white rounded-full text-xs transition-colors"
            >
              Order Online <ArrowRight size={11} />
            </Link>
          </div>
        </div>
      </div>

      {/* Content — same structure as specials page */}
      <div className="max-w-screen-xl mx-auto px-4 py-10">
        <div className="grid gap-6">
          {visibleSections.map((section, i) => (
            <SectionCard
              key={section.title}
              section={section}
              index={i}
              addedId={addedId}
              onAdd={handleAdd}
            />
          ))}
        </div>

        {/* CTA — same as specials */}
        <div className="mt-12 bg-stone-900 rounded-3xl p-8 flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="font-serif text-xl font-bold text-white mb-1">
              Pick Up at 939 N. 2nd Street, Philadelphia, PA 19123
            </h3>
            <p className="text-stone-400 text-sm leading-relaxed">{t("note")}</p>
          </div>
          <div className="flex gap-3 flex-wrap">
            <a
              href="tel:+12156275323"
              className="flex items-center gap-2 px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-full text-sm font-semibold transition-colors"
            >
              <Phone size={14} /> {t("callToOrder")}
            </a>
            <Link
              href={`/${locale}/ordering/guest`}
              className="flex items-center gap-2 px-6 py-3 bg-white/10 hover:bg-white/20 text-white rounded-full text-sm font-semibold transition-colors"
            >
              Order Online <ArrowRight size={14} />
            </Link>
          </div>
        </div>
      </div>
    </div>
  );
}

// ─── Section Card — identical structure to specials page ───────────────────────
function SectionCard({
  section, index, addedId, onAdd,
}: {
  section: DealSection;
  index: number;
  addedId: string | null;
  onAdd: (id: string, product: Product) => void;
}) {
  return (
    <div className="bg-white rounded-2xl border border-stone-100 shadow-sm overflow-hidden">
      {/* Header */}
      <div className="flex items-center gap-3 px-5 py-4 bg-stone-50 border-b border-stone-100">
        <span className="text-2xl">{section.icon}</span>
        <h2 className="font-serif text-lg font-bold text-stone-900 flex-1">{section.title}</h2>
        <span className="bg-red-100 text-red-700 text-xs font-bold px-3 py-1 rounded-full">
          {section.items.length} deals
        </span>
      </div>

      {/* Items — identical to specials item rows */}
      <div className="divide-y divide-stone-50">
        {section.items.map((item, idx) => {
          const product = dealItemToProduct(item, section, idx);
          const isAdded = addedId === product.id;
          const isMarket = item.price === "Market";
          return (
            <div
              key={`${item.name}-${idx}`}
              className="flex items-center gap-2 sm:gap-4 px-4 sm:px-5 py-3 hover:bg-stone-50 transition-colors"
            >
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-stone-800 leading-snug">{item.name}</p>
                {item.note && (
                  <p className="text-xs text-stone-400 mt-0.5">{item.note}</p>
                )}
                {item.hot && (
                  <span className="inline-block mt-0.5 bg-red-100 text-red-600 text-[10px] font-bold px-1.5 py-0.5 rounded-full uppercase">
                    Hot
                  </span>
                )}
              </div>
              <div className="flex items-center gap-2 shrink-0">
                {isMarket ? (
                  <span className="text-xs font-semibold text-red-500 italic whitespace-nowrap">Call for Price</span>
                ) : (
                  <span className="text-sm font-bold text-stone-900 whitespace-nowrap">
                    {item.price}
                    <span className="text-xs font-normal text-stone-400 ml-0.5">{item.unit}</span>
                  </span>
                )}
                <button
                  onClick={() => onAdd(product.id, product)}
                  className={cn(
                    "flex items-center gap-1 px-2.5 py-1.5 rounded-xl text-xs font-bold transition-all shrink-0",
                    isAdded ? "bg-green-500 text-white" : "bg-red-600 hover:bg-red-700 text-white"
                  )}
                >
                  {isAdded ? <><Check size={11} /> Added!</> : <><ShoppingCart size={11} /> Add</>}
                </button>
              </div>
            </div>
          );
        })}
      </div>
    </div>
  );
}
