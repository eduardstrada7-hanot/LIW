  ...
  return (
    <div className="min-h-screen bg-[#fafaf8]">
      ...
      <div className="max-w-screen-xl mx-auto px-4 py-6 sm:py-10">
        ...
        {/* CTA */}
        <div className="mt-8 sm:mt-12 bg-stone-900 rounded-2xl sm:rounded-3xl p-6 sm:p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
          <div>
          ...
          <div className="flex flex-wrap gap-2 sm:gap-3 shrink-0 w-full sm:w-auto">
            <a
              href="tel:+12156275323"
              className="flex items-center gap-2 px-5 py-2.5 sm:py-3 bg-red-600 hover:bg-red-700 text-white rounded-full text-sm font-semibold transition-colors"
            >
              <Phone size={14} /> {t("callToOrder")}
            </a>
            <Link
              href={`/${locale}/ordering/guest`}
              className="flex items-center gap-2 px-5 py-2.5 sm:py-3 bg-white/10 hover:bg-white/20 text-white rounded-full text-sm font-semibold transition-colors"
            >
...
// ─── Section Card — always expanded, matches specials page exactly ──────────────
      ...
      <div className="divide-y divide-stone-50">
        {section.items.map((item, idx) => {
          ...
          return (
            <div
              key={`${item.name}-${idx}`}
              className="flex items-center gap-2 sm:gap-4 px-4 sm:px-5 py-2.5 sm:py-3 hover:bg-stone-50 transition-colors"
            >
              ...
              <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                ...
                <button
                  ...
                  className={cn(
                    "flex items-center gap-1 px-2 sm:px-2.5 py-1.5 rounded-xl text-[10px] sm:text-xs font-bold transition-all shrink-0",
